            ...
            <div className="flex flex-col lg:flex-row justify-between relative z-10 gap-6 lg:gap-0">
              {steps.map((step, index) => (
                <div key={index} className="flex flex-col items-center text-center relative flex-1">
                  ...
                  {/* Connecting Arrow for mobile (Placed after content) */}
                  {index !== steps.length - 1 && (
                    <div className="lg:hidden w-px h-8 border-l-[1.5px] border-dashed border-[#de9e48]/50 mt-4 relative">
                       <svg className="w-3.5 h-3.5 text-[#de9e48]/50 absolute -bottom-2 -left-[6.5px] rotate-90" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                         <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M9 5l7 7-7 7" />
                       </svg>
                    </div>
                  )}
