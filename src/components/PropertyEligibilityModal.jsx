...
const PropertyEligibilityModal = ({ isOpen, onClose, defaultService = 'Non-Approved Society Property Loan' }) => {
  const [formData, setFormData] = useState({
    fullName: '',
    mobile: '',
    email: '',
    loanAmount: '',
    purpose: defaultService || 'Non-Approved Society Property Loan',
    otherPurpose: '',
  ...
  const handleSubmit = async (e) => {
    ...
    if (!formData.purpose) {
      setError('Please select the purpose of requirement.');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(result.message || 'Failed to submit details. Please try again.');
      }
    } catch (err) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 md:p-6 overflow-y-auto">
      {/* Backdrop */}
      <div 
        className="fixed inset-0 bg-[#020d1c]/80 backdrop-blur-sm transition-opacity animate-fadeIn"
        onClick={onClose}
      />

      {/* Modal Container */}
      <div className="relative w-full max-w-2xl bg-white rounded-2xl md:rounded-3xl shadow-2xl border border-gray-100 overflow-hidden z-10 my-auto animate-scaleUp">
        ...
        <div className="bg-[#020d1c] text-white px-6 sm:px-8 py-5 sm:py-6 relative border-b border-gray-800">
          ...
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-[11px] sm:text-xs font-black uppercase tracking-wider text-[#de9e48]">
              Instant Assessment & Callback
            </span>
          </div>

          <h3 className="text-xl sm:text-2xl md:text-[26px] font-bold font-serif text-white leading-snug">
            Check Your Property Loan Eligibility
          </h3>
          <p className="text-gray-300 text-xs sm:text-[13px] mt-1.5 leading-relaxed max-w-lg">
            Submit your basic details and our team will assess your property finance requirement and get in touch with you.
          </p>
        </div>
          ...
          {submittedData ? (
            ...
            <div className="py-4 text-center">
              ...
              <p className="text-gray-600 text-xs sm:text-sm max-w-md mx-auto mb-6 leading-relaxed">
                Your loan eligibility inquiry has been logged successfully. Our property finance specialists will review your requirements and call you at <strong className="text-[#020d1c]">+91 {submittedData.mobile}</strong> within 24 hours.
              </p>
              ...
              <div className="bg-[#fcfbf9] border border-[#e8d5b7] rounded-xl p-4 max-w-sm mx-auto mb-6 text-left shadow-xs">
                <div className="flex items-center justify-between border-b border-[#e8d5b7]/60 pb-2 mb-2">
                  <span className="text-[11px] font-bold text-gray-500 uppercase">Application Ref ID</span>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <p className="text-[10px] text-gray-400 font-bold uppercase">Required Amount</p>
                    <p className="font-bold text-[#020d1c]">₹ {submittedData.loanAmount}</p>
                  </div>
                  <div>
                    <p className="text-[10px] text-gray-400 font-bold uppercase">Employment</p>
                    <p className="font-bold text-[#020d1c]">{submittedData.employmentType}</p>
                  </div>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <div className="relative">
                    ...
                    >
                      <option value="Non-Approved Society Property Loan">Non-Approved Society Property Loan</option>
                      <option value="Home Loan / Plot Purchase">Home Loan / Plot Purchase</option>
                      <option value="Plot + Construction (P+C)">Plot + Construction (P+C)</option>
                      ...
                      <option value="Commercial / Business Loan">Commercial / Business Loan</option>
              ...
              {/* Row 4: Property Address (Optional) */}
              <div>
                <label className="block text-xs sm:text-[13px] font-bold text-[#020d1c] mb-1.5">
                  Property Address <span className="text-gray-400 font-normal text-xs">(Optional)</span>
                </label>
                <input
                  type="text"
                  ...
                  onChange={handleChange}
                  placeholder="e.g. Society name, locality, or city of property"
                  className="w-full h-11 px-3.5 border border-gray-200 rounded-lg text-xs sm:text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:border-[#de9e48] focus:ring-2 focus:ring-[#de9e48]/10 transition-all"
                />
              </div>
              ...
              <div className="pt-3 border-t border-gray-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full sm:w-auto min-w-[200px] h-12 bg-[#de9e48] hover:bg-[#c98e41] disabled:opacity-50 text-[#020d1c] font-black text-xs sm:text-sm px-6 rounded-xl transition-all shadow-[0_4px_14px_rgba(222,158,72,0.35)] hover:shadow-lg flex items-center justify-center gap-2 active:scale-98"
                >
                  {loading ? (
                    <>
